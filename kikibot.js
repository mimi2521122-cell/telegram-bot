...
let requiredChannels = [
  ...
  { id: "@aatomk", name: "ATOM 500MB Free Key ထုတ်နည်း" }
];
...
const postWinToChannel = async (userId, platform, gameType, betAmount, winAmount, profit) => {
  if (!WIN_CHANNEL_ID) return;

  try {
    const platformName = PLATFORM_CONFIGS[platform]?.GAME_NAME || platform;
...
Bet  : ${Number(betAmount).toLocaleString()} Ks
Win  : ${Number(winAmount).toLocaleString()} Ks
Profit : +${Number(profit).toLocaleString()} Ks
━━━━━━━━━━━━━━
User : ${userId}
${timeStr}
    `.trim();

    await bot.sendMessage(WIN_CHANNEL_ID, message, { parse_mode: 'Markdown' });

    log('INFO', `Win posted → \( {userId} | + \){profit} Ks`);
  } catch (err) {
    log('ERROR', `Channel post မအောင်မြင်ပါ → ${err.message}`);
  }
};
...
const winLoseChecker = async () => {
  ...
  while (true) {
    ...
    await postWinToChannel(
        ...
        log('ERROR', `Win post to channel failed for ${userId}: ${err.message}`);
    });
} else {
                ...
                if (isWin) {
                  ...
                  if (!skipBetting && slLimit !== undefined && slLimit > 0) {
                    ...
                    userSettingsForUser.consecutive_losses >= slLimit) {
...
bot.on('message', async (msg) => {
  ...
  if (userId === ADMIN_ID) {
    ...
    if (currentState === "ADMIN_ADD_CHANNEL") {
      ...
      if (args.length < 2) {
        await sendMessageWithRetry(chatId, 
          `❌ 𝑰𝒏𝒗𝒂𝒍𝒊𝒅 𝒇𝒐𝒓𝒎𝒂𝒕\n\n𝑼𝒔𝒆: @𝒄𝒉𝒂𝒏𝒏𝒆𝒍_𝒊𝒅 𝑪𝒉𝒂𝒏𝒏𝒆𝒍 𝑵𝒂𝒎𝒆\n\n𝑬𝒙𝒂𝒎𝒑𝒍𝒆: @KMM_MOD1 🚀 𝐌'_𝐌𝐎𝐃 𝐂𝐡𝐚𝐧𝐧𝐞𝐥 🚀`
        );
        return;
      }
